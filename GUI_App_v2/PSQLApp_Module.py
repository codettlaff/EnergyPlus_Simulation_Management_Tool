...
def get_generation_aggregation_zones(db_settings, simulation_id):
    ...
    try:
        ...
        # Get all zone_id and zone_name for this simulation
        cursor.execute("""
        ...
        valid_zone_ids = set(zone_id_to_name.keys())

        # Get all links in aggregation_zones table
        cursor.execute("""
            SELECT aggregation_zone_id, composite_zone_id
            FROM aggregation_zones;
        """)
        links = cursor.fetchall()

        # Extract sets of aggregation and composite zone IDs
        ...
        # Filter by zones that belong to this simulation
        generation_zone_ids = [zid for zid in generation_zone_ids if zid in valid_zone_ids]
        aggregation_zone_ids = [zid for zid in aggregation_zone_ids if zid in valid_zone_ids]

        # Build DataFrames
        generation_zones_df = pd.DataFrame(
            [(zid, zone_id_to_name[zid]) for zid in generation_zone_ids],
            columns=["zone_id", "zone_name"]
        )

        aggregation_zones_df = pd.DataFrame(
            [(zid, zone_id_to_name[zid]) for zid in aggregation_zone_ids],
            columns=["zone_id", "zone_name"]
        )

        return generation_zones_df, aggregation_zones_df

    except Exception as e:
        print(f"Error in get_generation_aggregation_zones: {e}")

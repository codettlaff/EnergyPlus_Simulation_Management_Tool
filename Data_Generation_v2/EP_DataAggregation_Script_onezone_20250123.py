...
# 20241025
# Aggregating zones to themselves - getting rid of unnecessary information
...
for Simulation_Name in Simulation_Name_List:
    ...
    print('Aggregating Data For: ', Simulation_Name, "\n")
    
    csv_filepath = os.path.join(results_folderpath, Simulation_Name, 'Sim_ProcessedData', 'Zone_Air_Temperature.csv')
    csv_column_list = get_csv_columns(csv_filepath)
    csv_column_list = csv_column_list[1:]
    zonelist = []
    for column_name in csv_column_list:
        zonelist.append(column_name.split(':')[0])
        
    # for each zone in zone list, check if it has node variables
    # if it does not, it is not part of building total area, and we do not want to include it in total building area.  
    csv_filepath = os.path.join(results_folderpath, Simulation_Name, 'Sim_ProcessedData', 'System_Node_Mass_Flow_Rate.csv')
    csv_column_list = get_csv_columns(csv_filepath)
    csv_column_list = csv_column_list[1:]
    
    thermal_zones = []
    for word1 in zonelist:
        for word2 in csv_column_list:
            if word1 in word2:
                if not (word1 in thermal_zones):
                    thermal_zones.append(word1)
    for zone in zonelist:
        if not (zone in thermal_zones):
            print(zone + ' is not a thermal zone.\n')
    
    # Aggregate all zones to a single zone
    Aggregation_Zone_List = []
    Aggregation_Zone_List.append(zonelist)

    Aggregation_Zone_NameStem = 'Aggregation_Zone_1Z'
    ...
    Sim_ProcessedData_FolderPath = os.path.join(results_folderpath, Simulation_Name, 'Sim_ProcessedData')
    ...
    Processed_BuildingSim_Data_FolderPath = results_folderpath
